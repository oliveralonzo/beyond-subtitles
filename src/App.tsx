...
export const App:FC = () => {
  ...
  // Setting for whether video uploads are allowed
  const [allowVideoUploads, setAllowVideoUploads] = useState(false)
  ...
  return <Container fluid>
    <Row>
      <Col id="video-player" className="flex flex-centered flex-column">
          <VideoPlayer player={ref} onVisualsChange={handleVisualsUpdate} playing={videoPlaying} allowVideoUploads={allowVideoUploads} setAllowVideoUploads={setAllowVideoUploads} onVideoLoad={handleVideoLoad} onProgress={handleVideoProgress} currEvents={currEvents} onPlayPause={handleVideoPlaying}/>
      </Col>
      <Col id="sound-events">
        <Container>
        <Tabs defaultActiveKey="sound-events" className="mb-3">
          ...
          <Tab eventKey="sound-events" title="Sound Events">
            <SoundEvents events={events} seekVideo={handleSeekVideo} currTime={currMilliseconds} onEventsChange={handleEventsChange} allowVideoUploads={allowVideoUploads} videoLoaded={videoLoaded} visuals={true} inputPlaceholder="Describe sound…"/>
          </Tab>
        </Tabs>
        </Container>
      </Col>
